# Honesty as a Marketing Strategy — Article Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add the article "Honesty as a Marketing Strategy" to the writing collection, appearing third on the `/writing` hub behind "Data is Killing Your Marketing" and "How to Signal in 2026".

**Architecture:** Articles live as markdown files in `src/content/writing/`. The Astro content collection auto-generates article pages via `src/pages/writing/[slug].astro`. The `/writing` hub (`src/pages/writing.astro`) shows the top 3 articles sorted by date descending. Display order is controlled entirely by the `date` frontmatter field.

**Tech Stack:** Astro 6 Content Collections, Markdown, Tailwind CSS v4, `ArticleLayout.astro` (handles h2/h3, blockquotes as pull quotes, images, links)

---

## Context: Date Ordering

The `/writing` hub sorts by `date` descending (newest first). Desired display order:

| Position | Article | Required Date |
|---|---|---|
| 1st | Data is Killing Your Marketing | Must be > 2026-04-06 → set to `2026-04-13` |
| 2nd | How to Signal in 2026 | Already `2026-04-06` — no change needed |
| 3rd | Honesty as a Marketing Strategy | Must be < 2026-04-06 → set to `2026-01-15` |

---

## Context: Image Mapping

Three image placeholders in the article → three images in `/public/`:

| Placeholder | Image File | Context |
|---|---|---|
| `[Insert "results equal" image]` | `/results.png` | After the Results=(work×time)^strategy formula |
| `[Insert "systems leverage" image]` | `/systems leverage.png` | After Meadows' leverage points discussion |
| `[Insert "results" image]` | `/optimism diagram.png` | After External Pessimism / Internal Optimism section |

Images with spaces in filenames must be URL-encoded in markdown: `/systems%20leverage.png`, `/optimism%20diagram.png`

---

## Context: Special Formatting

**Pull quotes** → standard markdown blockquote `> text` — `ArticleLayout.astro` already styles blockquotes as pull quotes (orange left border, italic, indigo background tint).

**H2/H3 headings** → standard markdown `##` / `###` — `ArticleLayout.astro` already styles these with Fraunces font and indigo color.

**Internal link** → `(I've written separately...)` parenthetical links to `/writing/data` (page not yet live but link is correct).

---

## Task 1: Fix display order — update date on "Data is Killing Your Marketing"

**Files:**
- Modify: `src/content/writing/data-is-killing-your-marketing.md` (line 4)
...
Open `src/content/writing/data-is-killing-your-marketing.md` and confirm frontmatter reads:
```markdown
---
...
```bash
git add src/content/writing/data-is-killing-your-marketing.md
git commit -m "fix: update date on Data article to control writing hub display order"
```

---

## Task 2: Create the new article file

**Files:**
- Create: `src/content/writing/honesty-as-a-marketing-strategy.md`

**Step 1: Create the file with the full content below**

````markdown
---
title: "Honesty as a Marketing Strategy"
...
![The results formula: Results=(work×time)^strategy](/results.png)
...
![Donella Meadows' systems leverage points](/systems%20leverage.png)
...
![External pessimism and internal optimism diagram](/optimism%20diagram.png)
...
Confirm `src/content/writing/honesty-as-a-marketing-strategy.md` exists and the frontmatter is valid.

**Step 3: Commit**

```bash
git add src/content/writing/honesty-as-a-marketing-strategy.md
git commit -m "feat: add Honesty as a Marketing Strategy article"
```
...
**Step 2: Check the writing hub**

Navigate to `http://localhost:4321/writing` and confirm:
- 3 article cards visible
- Order: Data is Killing Your Marketing → How to Signal in 2026 → Honesty as a Marketing Strategy

**Step 3: Check the article page**

Navigate to `http://localhost:4321/writing/honesty-as-a-marketing-strategy` and confirm:
- All H2/H3 headings render with correct Fraunces font styling
- 3 images appear (results.png, systems leverage.png, optimism diagram.png)
- 2 blockquote pull quotes are styled with orange left border
- All external links are clickable (Meadows list, skilled incompetence, know now, base-rates, planning fallacy)
- Internal link `/writing/data` is present in the Short-termism section
- Footnote/attribution paragraph at bottom renders correctly

**Step 4: Commit if any fixes were needed**

```bash
git add -p
git commit -m "fix: correct article rendering issues"
```
